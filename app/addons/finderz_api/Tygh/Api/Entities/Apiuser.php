...
class Apiuser extends AEntity
{
    ...
    public function create($params)
    {
        ...
        $user_id = 0;
        if(isset($params['recovery']) && $params['recovery'] == true){
                ...
                if ($valid_params) {
                    $rs = fn_recover_password_generate_key($params['email']);
                    if(!empty($rs)){  
                        $status = Response::STATUS_OK;
            ...
            if ($valid_params) {
                list($user_id, $profile_id) = fn_update_user($user_id, $params, $auth, false, true);
    ...
    public function update($id, $params)
    {
        ...
        $status = Response::STATUS_NOT_FOUND;
        if($id){
            $user_data = fn_get_user_short_info($id);
            if(!empty($user_data)){
                if($user_data['email'] == $params['email']){
                   $rs = fn_recover_password_generate_key($user_data['email']);
                    $status = Response::STATUS_OK;
                    $data = 'Recovery email sent';
                }
                else{
